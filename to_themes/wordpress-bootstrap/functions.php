...
add_db_table_editor(array( 'title'=>'Doświadczenie zawodowe', 'table'=>'employment')); 
add_db_table_editor(array( 'title'=>'Projekty', 'table'=>'portfolio')); 
add_db_table_editor(array( 'title'=>'Technologie', 'table'=>'tech')); 
add_db_table_editor(array( 'title'=>'Plan zajęć', 'table'=>'timetable', 'auto_date'=>false)); 

function edit_admin_menus() {
        global $menu;
        global $submenu;
        // var_dump($submenu);
        $submenu["wp-db-table-editor"][0][0] = "Menu edycji danych"; 
        $menu[50][0] = 'Edycja danych użytkownika';
}
add_action( 'admin_menu', 'edit_admin_menus' );

function  handle_header_shortcode($atts)
{
    return '
        <div class="jumbotron">
        <div class="row placeholders">
        <div class="col-xs-6 col-sm-3 placeholder">
        <img src="http://students.mimuw.edu.pl/~mg304742/photo.jpg" width="200">
        <span class="text-muted"</span>
        </div>
        <div class="col-xs-6 col-sm-9 placeholder">
        <h1>Michał Garmulewicz</h1>
        <p>  Jestem młodym matematykiem i programistą. Lorem ipsum dolor sit
        amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore
        et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation
        ullamco laboris nisi ut aliquip ex ea commodo consequat.  </p>
        <p>Mieszkam w Warszawie.</p>
        <p>garmulewicz@gmail.com </p>
        <p>+48 794 11 99 58</p>
        </div>
        </div>
        </div>
...
add_shortcode('cv_header', 'handle_header_shortcode');

function  handle_current($atts)
{
    $resp = '
           <div class="page-header">
             <center>
              <h1>Obecnie realizowane przedmioty</h1>
             </center>
           </div>
           <div class="row">
             <div class="col-md-1">
             ...
             <div class="col-md-10">
               <table class="table table-striped">
                 ...
                 ';
      global $wpdb;
      $result = $wpdb->get_results ( "SELECT * FROM current" );
...
add_shortcode('current_courses', 'handle_current');


function  handle_grades($atts)
{
  $resp =  '
 <div class="page-header">
   <center>
 ...
 <div class="row">
   ...
   <div class="col-md-10">
     <table class="table table-striped">
       ...
       ';
          global $wpdb;
           $result = $wpdb->get_results ( "SELECT * FROM grades" );
           foreach ( $result as $print )   {
               $resp = $resp . '
         <tr> 
           <td> ' . $print->id . '</td>
...
add_shortcode('grades', 'handle_grades');



function  handle_portfolio($atts)
{
 ...
 <div class="row">
   ...
   <div class="col-md-10">
     <table class="table table-striped">
       ...
       <tbody> ';
      global $wpdb;
             $result = $wpdb->get_results ( "SELECT * FROM portfolio" );
             foreach ( $result as $print )   {
                 $resp = $resp . ' <tr> 
                     <td> ' . $print->id . '</td>
                     <td> ' . $print->name . '</td>
...
add_shortcode('project_portfolio', 'handle_portfolio');


function  handle_tech($atts)
{
    $resp = '
 <div class="page-header">
   <center>
   ...
   <div class="col-md-10">
     <table class="table table-striped">
       ...
       <tbody>
        ';
    global $wpdb;
           $result = $wpdb->get_results ( "SELECT * FROM tech" );
      foreach ( $result as $print )   {
                   $resp = $resp . '<tr> 
                     <td>' . $print->id . ' </td>
                     <td>' . $print->name . ' </td>
                     <td>' . $print->skill_level . ' </td>
                     <td>' . $print->link . ' </td>
                   </tr>';
       }
         $resp = $resp . '
...
add_shortcode('tech', 'handle_tech');


function  handle_jobs($atts)
{
    $resp = '
 <div class="page-header">
   <center>
   ...
   <div class="col-md-10">
     <table class="table table-striped">
       ...
       <tbody>
        ';
    global $wpdb;
           $result = $wpdb->get_results ( "SELECT * FROM employment" );
      foreach ( $result as $print )   {
                   $resp = $resp . '<tr> 
                     <td>' . $print->id . ' </td>
                     <td>' . $print->company . ' </td>
...
add_shortcode('employment', 'handle_jobs');


function  handle_awards($atts)
{
    $resp = '
 <div class="page-header">
   <center>
   ...
   <div class="col-md-10">
     <table class="table table-striped">
       ...
       <tbody>
        ';
    global $wpdb;
           $result = $wpdb->get_results ( "SELECT * FROM awards" );
      foreach ( $result as $print )   {
                   $resp = $resp . '<tr> 
                     <td>' . $print->id . ' </td>
                     <td>' . $print->name . ' </td>
                     <td>' . $print->datereceived . ' </td>
                     <td>' . $print->descr . ' </td>
                     <td>' . $print->link . ' </td>
                   </tr>';
       }
...
add_shortcode('awards', 'handle_awards');
